        ...
        for s in sqs:
            for c in cams:
                ...
                if set == 'test':
                    ...
                    data.build_ts_df(6, 19, [7], s, cams=c, clear_sky_label=False)
                ...
                if set == 'test':
                    lstm = lstm_model_multi.LSTM_predictor(data, 100,
                                                    'LSTM_TEST_PXL' + name_epoch + name_time + name_data + name_cam, pred_csi=False)
                    lstm.set_days(data.get_thesis_test_days())
                else:
                    lstm = lstm_model_multi.LSTM_predictor(data, 100,
                                                    'LSTM_PREM_PXL' + name_epoch + name_time + name_data + name_cam, pred_csi=False)
